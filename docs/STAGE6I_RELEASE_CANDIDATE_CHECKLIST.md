...
| `npm run preflight:stage6h -- --expect-commit <commit>` | Same as above, plus deployed commit check through `/build-meta.json`; `<commit>` may be a full 40-character SHA or a 7+ character hex prefix. | NO |
...
| `node tests\stage6i\stage-status-docs.test.cjs` | PASS | Guards Stage 6 status docs so true-device items remain `MANUAL_REQUIRED`, stale Final-Fix deployment wording cannot return, and Stage 6I docs-only commit handling stays explicit. |
...
| `npm run precheck:stage6i -- --expect-commit f99a262 --defer-manual-result` | PASS_DEVELOPMENT_MANUAL_DEFERRED | Runtime-code continuation audit passed with `precheck_ok=true`, `ok=true`, `can_continue_development=true`, `manual_result_deferred=true`, `formal_gate_ok=false`, `can_enter_stage6i=false`, `api_calls_made=0`, `quota_consumed=false`, `real_qwen_called=false`; `/build-meta.json` matched `f99a2622b2def593a6911edf4d95ffd40bb1cd6b`. Later docs-only commits only need deployment pointer preflight. This is not a final release PASS because true-device evidence is still deferred. |
...
- `f99a2622b2def593a6911edf4d95ffd40bb1cd6b` - Stage 6F abnormal image fixture coverage; workers.dev matched this commit through `/build-meta.json`.
- `cdadccddcd56ebce9a2adf725e9c50d6477066fe` - accept 7+ character short commit prefixes in Stage 6H preflight.
- `360e78cb6a8bf0fe493c0a8a040bbe7bee8132bf` - refresh Stage 6I zero-cost audit documentation.
