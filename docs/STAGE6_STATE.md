...
Stage 6G：CONDITIONAL_PASS。

Stage 6F status: CONDITIONAL_PASS

Stage 6G status: CONDITIONAL_PASS

Reason: 2026-05-31 Stage 6G 已补齐上线前最小稳定性、错误提示、轻量限流、成本保护和日志最小化保护；`npm run test:stage5p`、`npm run build`、`node scripts\stage6f\security-scan.cjs`、`npm run smoke:stage6f:qwen`、`npm run test:stage6f`、`npm test` 均通过。Qwen smoke 为 dry run，`api_calls_made=0`；`test:stage6f` 和 `npm test` 各包含一次生产正常掌纹上传 E2E，均返回 HTTP 200、`provider=qwen`、有 `analysis_result`，`/result/` 和 `/poster/` 可读取结果。Stage 6G 标记为 CONDITIONAL_PASS，而不是无条件 PASS，原因是新代码仍需随本提交部署后在生产环境实际生效，且 iPhone 微信、Android 微信、iPhone Safari 真机和 Android Chrome 真机验收仍需用户确认，Codex 未伪造人工真机结果。
