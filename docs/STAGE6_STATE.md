...
- Stage 6I：Release Candidate 清单已准备，状态 BLOCKED_BY_STAGE6H_MANUAL_REQUIRED。
  ...
  - Stage 6I 聚合预检入口已准备并验证：`npm run precheck:stage6i -- --expect-commit <latest-origin-main-commit> --manual-result-file <Codex-saved-user-result-text> --require-manual-result`；该入口拒绝真实 Qwen env，只对 `npm test` / 线上 preflight 的明确零成本网络瞬断做有限重试，正式门禁缺少真机回填文件或 expected commit 时 fail-fast，真机回填不足时返回 `formal_gate_ok=false` 并 fail-safe。
  - 预检查未调用真实 Qwen，未消耗额度；Stage 6I 正式开始时仍需重新运行这些命令。
  - Stage 6I 不能在 Stage 6H 微信双端最低真机门槛缺失时标记 PASS / CONDITIONAL_PASS；Safari / Chrome 未测项只能作为 MANUAL_REQUIRED 风险保留。
