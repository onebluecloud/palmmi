...
Stage 6F-Fix-4：Stabilize NOT_PALM Gate and Prevent Invalid Image Timeout 已完成代码修复和自动化回归。

Stage 6F-Fix-4 status: CODE_FIXED_MANUAL_RETEST_REQUIRED

Stage 6G: BLOCKED

Reason: 用户在 Stage 6F-Fix-3 部署后的安卓微信真机复测中确认非手掌图没有再出人格，但仍返回 `REQUEST_TIMEOUT` 而不是 `NOT_PALM`；本轮已通过代码和自动化把 Qwen provider 改为有效性优先预检，非手掌不进入人格分析，但尚未获得修复后安卓微信真机复测结果，iOS 微信也仍未真机验收。
...
条件：只有在本次 Fix-4 部署后，安卓微信拍照上传、相册上传、非手掌稳定 `NOT_PALM` 且不超时、多手掌不塌缩 P25、海报生成真机复测通过，且继续保持无 Key / Token / base64 / raw response 泄露，才允许重新评估 Stage 6G。
