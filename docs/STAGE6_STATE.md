...
Reason: 2026-05-31 Codex 已新增并更新 `docs/STAGE6H_REAL_DEVICE_QUICK_TEST_PACKET.md`，把 iPhone Safari、iPhone 微信、Android Chrome、Android 微信真机验收压缩成非技术用户可直接复制回填的步骤和模板。速查包现在包含“最小微信双端回填模板”：用户可先完成 iPhone 微信 + Android 微信，Safari / Chrome 未测项继续保留为 `MANUAL_REQUIRED`，不会被伪造成 PASS。该文件不会替代真实测试结论；Stage 6H 仍为 `MANUAL_REQUIRED`。Cloudflare 最新部署 commit 以 Codex 最终报告和 `/build-meta.json` 预检输出为准，优先由 `npm run preflight:stage6h -- --expect-commit <latest-origin-main-commit>` 自动确认，Dashboard 仅作为命令失败时的兜底。
