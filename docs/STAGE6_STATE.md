...
- Stage 6I：Release Candidate 清单已准备，状态 BLOCKED_BY_STAGE6H_MANUAL_REQUIRED。
  ...
  - Stage 6I 聚合预检入口已准备并验证：`npm run precheck:stage6i -- --expect-commit <latest-origin-main-commit> --manual-result-file <Codex-saved-user-result-text> --require-manual-result`；该入口拒绝真实 Qwen env，正式门禁缺少真机回填文件时 fail-fast，真机回填不足时返回 `formal_gate_ok=false` 并 fail-safe。
