...
Stage 6F-Real-Qwen-Smoke：新增真实 Qwen API 小样本验收脚本，用于人工显式验证当前 prompt / parser / contract 在真实 Qwen 返回下是否成立。

Stage 6F-Real-Qwen-Smoke status: READY_NOT_RUN

Stage 6G: BLOCKED

Reason: 当前真实微信复测显示啤酒 / 饮料图已能返回 `NOT_PALM`，但真实手掌图仍可能被误杀为 `ANALYSIS_UNRELIABLE`；本轮只新增人工显式运行的真实 Qwen 小样本 smoke 脚本，默认不调用真实 API。尚未获得该 smoke 真实运行结果、安卓微信复测结果和 iOS 微信真机验收结果，Stage 6G 继续 BLOCKED。
...
| Real Qwen smoke result | NOT_RUN | Codex 未默认调用真实 Qwen；需用户显式加 `--real` |
...
| Fix-4 部署后生产真实上传确认 | MANUAL_RETEST_REQUIRED | 当前代码修复后需等待 Pages 部署并复测 |
...
| Real Qwen smoke 真实小样本结果 | NOT_RUN | 需要用户显式执行 `npm run smoke:stage6f:qwen -- --real ...` 并提供脱敏 summary |
...
条件：只有在本次 smoke 脚本完成真实 Qwen 小样本验证、Fix-4 部署后安卓微信拍照上传 / 相册上传 / 非手掌稳定 `NOT_PALM` 且不超时 / 多手掌不塌缩 P25 / 海报生成真机复测通过，且继续保持无 Key / Token / base64 / raw response 泄露，才允许重新评估 Stage 6G。
